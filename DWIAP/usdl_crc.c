...
{
    return GetCRC16_CCITT(seed, src, length);
}
...
u16 GetCRC16_IBM(u16 seed, u8* src, u32 len)
{
    ...
    while(len--)
    {
        r = (r >> 8) ^ CrctableIBMRefl[(r ^ *src++) & 0xFF];
    }
...
* in the FTP archive "ftp.adelaide.edu.au/pub/rocksoft".
***************************************************************************************************/
const u16 CrctableCCITTRefl[256];
...
u16 GetCRC16_CCITT(u16 seed, u8* src, u32 len)
{
    ...
    while(len--)
    {
        r = (r >> 8) ^ CrctableCCITTRefl[(r ^ *src++) & 0xFF];
    }
...
/*CRC LOOKUP TABLE DECLARATION*/
extern const u32 CRC32ReverseTable[256];
...
u32 GetCRC32(u32 seed, u8* src,u32 len)
{
    ...
    for (i = 0; i < len; i++)
    {
        crc = ((crc >> 8) & 0x00FFFFFF) ^ CRC32ReverseTable[(crc ^ *src++) & 0xFF];
    }
    return crc;
}
...
*\Parameter     len     缓冲区长度
*\Return        u32     校验值
*\Note          种子为0xFFFFFFFF，多项式为0xEDB88320，最终校验结果会取做一次位取反操作。
*\Note          应用于Ethernet,MPEG-2,PNG,POSIX cksum等。
*\Log           2010.01.11    Ver 1.0    高峰
*               创建函数。
***************************************************************************************************/
...
/*Poly: 0xEDB88320*/
const u32 CRC32ReverseTable[256] = {
